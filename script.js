document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (value !== '') {
        ...
        if (tagName === 'textarea' && field.dataset.minLength && value.length < parseInt(field.dataset.minLength)) {
          showFieldError(field, 'Minimum ' + field.dataset.minLength + ' characters required.');
          return false;
        }
      }
  ...
  if (animatableElements.length > 0) {
    ...
    style.textContent = [
      ...
      '.section-title, .feature-card, .service-card, .testimonial-card,',
      '.pricing-card, .team-card, .blog-card, .cta-block, .stat-item, .gallery-item {',
      '  opacity: 0;',
      ...
      '.testimonial-card.is-visible, .pricing-card.is-visible, .team-card.is-visible,',
      '.blog-card.is-visible, .cta-block.is-visible, .stat-item.is-visible,',
    ...
    if ('IntersectionObserver' in window) {
      ...
      animatableElements.forEach(function (el, index) {
        if (!el.dataset.delay && !el.dataset.animateDelay) {
          const siblings = el.parentNode ? el.parentNode.querySelectorAll(ANIMATION_SELECTORS.join(', ')) : [];
          let siblingIndex = 0;
          siblings.forEach(function (sibling, i) {
            if (sibling === el) siblingIndex = i;
          });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "פרוצס PROCES משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'ec23bc7b-a235-4ee6-ac0c-a85f04379e52',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'ec23bc7b-a235-4ee6-ac0c-a85f04379e52';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ec23bc7b-a235-4ee6-ac0c-a85f04379e52';
